  ...
  <div v-else class="weather-wrapper">
    <div class="weather-main" v-if="!getError && isSearched">
      ...
      <div class="weather-temp">
        <div
          class="weather-icon"
          :style="[icon ? { 'background-image': 'url(http://openweathermap.org/img/wn/' + icon + 'd@2x.png)' } : {}]"
        ></div>
        <span>{{ temp }}</span>
        <sup>&deg;</sup>
      </div>
      <div class="weather-description">{{ description }}</div>
      <button type="button" @click="getChart({ city, isChartShow })">
        {{ checkCityOfChart ? "Hide Chart" : "Show Chart" }}
      </button>
      ...
      <Modal v-show="showModal" @close="onClose">
        ...
        <div class="buttons-wrapper">
          <button @click="onDelete(choosedIndex)">Yes</button>
...
export default {
  ...
  methods: {
    ...
    onDelete(index) {
      this.deleteWeatherCard(index);
      this.onClose();
    },
    getChart({ city, isChartShow }) {
      this.getRequestChart({ city, isChartShow });
